...
### 2026-01-11 - 프리셋 색상 리팩토링 및 초기 색상 개선
...
1. **PRESET_COLORS 상수화**:
   - 새 파일 생성: `client/src/screen_party_client/gui/constants.py`
   - 튜플 `("이름", QColor)` → 단순 `QColor` 리스트로 변경
   - `get_default_pen_color()` 헬퍼 함수 추가
   - 코드 재사용성 향상

2. **펜 초기 색상 파스텔 핑크로 변경**:
   - 기존: 빨강 (255, 0, 0)
   - 변경: 파스텔 핑크 (255, 182, 193) - 첫 번째 프리셋
   - `DrawingCanvas`: 기본값 `_get_default_pen_color()` 사용
   - `OverlayWindow`: 기본값 파스텔 핑크로 변경
   - `MainWindow`: `get_default_pen_color()` 사용

3. **GUI 색상 버튼 개선**:
   - 버튼 텍스트(이름) 제거
   - 정사각형 버튼 (40x40)으로 변경
   - 색상만 표시하여 시각적으로 깔끔
   - 테두리 추가 (`border: 2px solid #888`)

4. **테스트 업데이트**:
   - 초기 색상이 파스텔 핑크로 변경된 것 반영
   - 9개 테스트 모두 통과
...
> - 초기 색상이 파스텔 핑크(첫 번째 프리셋)로 설정됩니다
> - 색상 버튼은 이름 없이 색상만 표시하여 깔끔합니다
> - PRESET_COLORS는 constants.py에 상수로 정의되어 있습니다
> - 참여자 중복 버그가 수정되었습니다
