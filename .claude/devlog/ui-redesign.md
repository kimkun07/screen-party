...
- [x] 메인 화면에 복사 버튼 추가
- [x] pytest-qt를 사용한 GUI 자동화 테스트 작성 (17개 테스트 모두 통과)
...
**테스트 결과**:
- ✅ Python 문법 체크 통과
- ✅ **pytest-qt GUI 자동화 테스트 17개 모두 통과** (offscreen 모드)
- ✅ 통합 테스트는 WebSocketClient를 직접 사용하므로 영향 없음

**추가된 GUI 테스트**:
1. 시작 화면 테스트 (6개)
   - 초기 상태, 서버 주소 기본값/커스텀, 세션 입력 시 접속 버튼 활성화
2. 메인 화면 테스트 (3개)
   - 메인 화면 표시, 서버 주소 복사, 세션 번호 복사
3. UI 상태 테스트 (3개)
   - 버튼 활성화/비활성화
4. 상태 메시지 테스트 (3개)
   - 시작/메인 화면 상태 메시지
